...
const normalizeChannelGroupConfigs = (
  groups = [],
  configs = [],
  defaultPriority = 0,
  defaultWeight = 0,
  ...
  const configMap = new Map(
    (Array.isArray(configs) ? configs : [])
      .map((config) => [String(config?.group || '').trim(), config])
      .filter(([group]) => group),
  );
  return normalizeChannelGroups(groups).map((group) => {
    const config = configMap.get(group) || {};
    return {
      group,
      priority:
...
const EditChannelModal = (props) => {
  ...
  const unitLoadRequestedRef = useRef(false);
  ...
  const [unitAccountList, setUnitAccountList] = useState([]);
  const [unitAccountOptions, setUnitAccountOptions] = useState([]);
  const [unitLoading, setUnitLoading] = useState(false);
  const [unitAccountLoading, setUnitAccountLoading] = useState(false);
  const [unitPlatformTokens, setUnitPlatformTokens] = useState([]);
  const [unitPlatformTokenLoading, setUnitPlatformTokenLoading] =
    useState(false);
  const [unitPlatformTokenModalVisible, setUnitPlatformTokenModalVisible] =
    useState(false);
  const [selectedUnitPlatformTokenKey, setSelectedUnitPlatformTokenKey] =
    useState('');
  const [unitPlatformTokenCreateVisible, setUnitPlatformTokenCreateVisible] =
    useState(false);
  const [unitPlatformTokenCreating, setUnitPlatformTokenCreating] =
    useState(false);
  const [unitPlatformTokenOptions, setUnitPlatformTokenOptions] = useState({
    groups: [],
    models: [],
  });
  const unitPlatformTokenCreateFormApiRef = useRef(null);
  ...
  const handleInputChange = (name, value) => {
    ...
    if (name === 'groups' && Array.isArray(value)) {
      const groups = normalizeChannelGroups(value);
      setInputs((inputs) => ({
        ...inputs,
        groups,
        group_configs: normalizeChannelGroupConfigs(
          groups,
          inputs.group_configs,
          inputs.priority,
          inputs.weight,
        ),
      }));
  ...
  const handleGroupConfigChange = (group, field, value) => {
    setInputs((inputs) => ({
      ...inputs,
      group_configs: normalizeChannelGroupConfigs(
        inputs.groups,
        (inputs.group_configs || []).map((config) =>
          config.group === group
  ...
  const formatUnitOptionLabel = (unit) => {
    const parts = [unit.name || `#${unit.id}`];
    if (unit.type) {
      parts.push(unit.type);
    }
    if (Number(unit.account_count || 0) > 0) {
      parts.push(`${unit.account_count} ${t('账号')}`);
    }
    if (unit.effective_api_url) {
      parts.push(unit.effective_api_url);
    }
    return parts.join(' · ');
  };

  const formatUnitAccountOptionLabel = (account) => {
    const primary =
      account.account || account.account_id || `${t('账号')} #${account.id}`;
    return account.account_id && account.account_id !== primary
      ? `${primary} · ${account.account_id}`
      : primary;
  };

  const loadUnits = async (force = false) => {
    if (unitLoadRequestedRef.current && !force) {
      return;
    }
    unitLoadRequestedRef.current = true;
    setUnitLoading(true);
    try {
      ...
      if (!success) {
        ...
        unitLoadRequestedRef.current = false;
        return;
      }
      setUnitList(data.items || []);
      setUnitOptions(
        (data.items || []).map((unit) => ({
      ...
      setUnitOptions([]);
      setUnitAccountList([]);
      setUnitAccountOptions([]);
      if (error?.response?.status === 404) {
        showWarning(t('单位接口暂不可用，已跳过单位选择'));
      } else {
        showError(getRequestErrorMessage(error, t('获取单位列表失败')));
      }
      unitLoadRequestedRef.current = false;
    } finally {
  ...
  const loadUnitAccounts = async (unitId) => {
    ...
    try {
      ...
      setUnitAccountList(data || []);
      setUnitAccountOptions(
        (data || []).map((account) => ({
          label: formatUnitAccountOptionLabel(account),
          value: account.id,
        })),
      );
    } catch (error) {
      ...
      setUnitAccountOptions([]);
      if (error?.response?.status === 404) {
        showWarning(t('单位账号接口暂不可用，已跳过账号选择'));
      } else {
        showError(getRequestErrorMessage(error, t('获取单位账号失败')));
      }
    } finally {
      setUnitAccountLoading(false);
    }
  };

  const ensureUnitsLoaded = (force = false) => {
    if (!unitLoading && (force || unitOptions.length === 0)) {
      loadUnits(force).then();
    }
  };
  ...
  const getUnitApiURL = (unitId) => {
    const unit = unitList.find((item) => item.id === Number(unitId || 0));
    if (!unit) return '';
    return (
      unit.effective_api_url ||
      unit.api_url ||
      unit.website_url ||
      ''
    ).trim();
  ...
  const applyUnitAccountDefaults = (unitAccountId) => {
    const account = unitAccountList.find(
      (item) => item.id === Number(unitAccountId || 0),
    );
    if (!account) return;
    const apiURL = getUnitApiURL(account.unit_id || inputs.unit_id);
    if (apiURL) {
      handleInputChange('base_url', apiURL);
    }
  };

  const clearUnitPlatformTokenState = () => {
    setUnitPlatformTokens([]);
    setSelectedUnitPlatformTokenKey('');
    setUnitPlatformTokenOptions({ groups: [], models: [] });
  };

  const handleUnitChange = (value) => {
    const nextUnitId = Number(value || 0);
    handleInputChange('unit_id', nextUnitId || undefined);
    handleInputChange('unit_account_id', undefined);
    setUnitAccountList([]);
    setUnitAccountOptions([]);
    clearUnitPlatformTokenState();
    const apiURL = getUnitApiURL(nextUnitId);
    if (apiURL) {
      handleInputChange('base_url', apiURL);
    }
    if (nextUnitId) {
      loadUnitAccounts(nextUnitId).then();
    }
  };

  const handleUnitAccountChange = (value) => {
    const nextUnitAccountId = Number(value || 0);
    handleInputChange('unit_account_id', nextUnitAccountId || undefined);
    clearUnitPlatformTokenState();
    applyUnitAccountDefaults(nextUnitAccountId);
  };

  const getUnitPlatformTokenLabel = (token) => {
    const parts = [token.name || `${t('令牌')} #${token.id}`];
    if (token.group) {
      parts.push(`${t('分组')}: ${token.group}`);
    }
    if (token.model_limits_enabled && token.model_limits) {
      parts.push(`${t('模型')}: ${token.model_limits}`);
    }
    return parts.join(' · ');
  };

  const getUnitPlatformTokenCreateInitValues = () => ({
    name: `channel-token-${(() => {
      const now = new Date();
  ...
  const loadUnitPlatformTokenOptions = async () => {
    const accountId = Number(inputs.unit_account_id || 0);
    const unitId = Number(inputs.unit_id || 0);
    if (!unitId || !accountId) {
      return;
    }
    try {
      const res = await API.get(
        `/api/unit/${unitId}/accounts/${accountId}/token_options`,
      );
      const { success, data, message } = res.data;
      if (!success) {
        showError(message || t('获取平台令牌选项失败'));
        return;
      }
      setUnitPlatformTokenOptions({
        groups: data?.groups || [],
        models: data?.models || [],
      });
    } catch (error) {
      showError(getRequestErrorMessage(error, t('获取平台令牌选项失败')));
    }
  };

  const openUnitPlatformTokenModal = async () => {
    const accountId = Number(inputs.unit_account_id || 0);
    const unitId = Number(inputs.unit_id || 0);
    if (!unitId) {
      showInfo(t('请先选择所属单位'));
    ...
    setUnitPlatformTokenLoading(true);
    try {
      const res = await API.get(
        `/api/unit/${unitId}/accounts/${accountId}/tokens`,
      );
      const { success, data, message } = res.data;
      if (!success) {
        showError(message || t('获取平台令牌失败'));
        return;
      }
      const tokens = data || [];
      setUnitPlatformTokens(tokens);
      setSelectedUnitPlatformTokenKey('');
      if (tokens.length === 0) {
        showInfo(t('该账号暂无可用平台令牌'));
      }
      loadUnitPlatformTokenOptions().then();
      setUnitPlatformTokenModalVisible(true);
    } catch (error) {
      showError(getRequestErrorMessage(error, t('获取平台令牌失败')));
    } finally {
      setUnitPlatformTokenLoading(false);
    }
  };

  const openCreateUnitPlatformTokenModal = () => {
    loadUnitPlatformTokenOptions().then();
    setUnitPlatformTokenCreateVisible(true);
    setTimeout(() => {
      unitPlatformTokenCreateFormApiRef.current?.setValues(
        getUnitPlatformTokenCreateInitValues(),
      );
    }, 0);
  };

  const submitCreateUnitPlatformToken = async (values) => {
    const accountId = Number(inputs.unit_account_id || 0);
    const unitId = Number(inputs.unit_id || 0);
    if (!unitId || !accountId) {
      showInfo(t('请先选择所属单位和所属账号'));
      return;
    }
    ...
    setUnitPlatformTokenCreating(true);
    try {
      ...
      const { success, data, message } = res.data;
      if (!success) {
        showError(message || t('创建平台令牌失败'));
        return;
      }
      const token = data;
      setUnitPlatformTokens((items) => [token, ...items]);
      setSelectedUnitPlatformTokenKey(String(token?.key || '').trim());
      setUnitPlatformTokenCreateVisible(false);
      showSuccess(t('平台令牌已创建并选中'));
    } catch (error) {
      showError(getRequestErrorMessage(error, t('创建平台令牌失败')));
    } finally {
      setUnitPlatformTokenCreating(false);
    }
  };

  const applySelectedUnitPlatformToken = () => {
    const key = String(selectedUnitPlatformTokenKey || '').trim();
    if (!key) {
      showInfo(t('请先选择平台令牌'));
      return;
    }
    handleInputChange('key', key);
    setUnitPlatformTokenModalVisible(false);
    showSuccess(t('已填入平台令牌'));
  };

  const renderSelectUnitPlatformTokenButton = () => (
    <Button
      size='small'
      type='primary'
      theme='light'
      onClick={openUnitPlatformTokenModal}
      disabled={!Number(inputs.unit_account_id || 0)}
      loading={unitPlatformTokenLoading}
    >
      {t('获取平台令牌')}
    </Button>
  );
  ...
  const loadChannel = async () => {
    ...
    if (success) {
      ...
      if (data.group === '') {
        ...
        data.groups = normalizeChannelGroups(data.group.split(','));
      }
      data.group_configs = normalizeChannelGroupConfigs(
        data.groups,
        data.group_configs,
        data.priority,
        data.weight,
      ...
      data.unit_id = data.unit_id ? Number(data.unit_id) : undefined;
      data.unit_account_id = data.unit_account_id
        ? Number(data.unit_account_id)
        : undefined;
      loadUnitAccounts(data.unit_id).then();
  ...
  }, [props.visible, props.editingChannel.id]);
  ...
  const submit = async () => {
    ...
    localInputs.unit_id = Number(localInputs.unit_id || 0);
    localInputs.unit_account_id = Number(localInputs.unit_account_id || 0);
    ...
    const normalizedGroups = normalizeChannelGroups(localInputs.groups || []);
    localInputs.group = normalizedGroups.join(',');
    localInputs.group_configs = normalizeChannelGroupConfigs(
      normalizedGroups,
      localInputs.group_configs,
      localInputs.priority,
      localInputs.weight,
    );
  ...
  const renderBaseURLField = () => {
    ...
    if (inputs.type === 45 && !doubaoApiEditUnlocked) {
      return (
        <Form.Select
          field='base_url'
          label={t('API地址')}
  ...
  return (
    <>
                ...
                <Spin spinning={loading}>
                  <div className='p-2 space-y-3' ref={formContainerRef}>
                    ...
                    <Card className='!rounded-2xl shadow-sm border-0'>
                      ...
                      <Row gutter={12}>
                        <Col span={12}>
                          <div onMouseDown={() => ensureUnitsLoaded(true)}>
                            <Form.Select
                              field='unit_id'
                              label={t('所属单位')}
                              ...
                              filter={selectFilter}
                              showClear
                              onFocus={() => ensureUnitsLoaded(true)}
                        ...
                        <Col span={12}>
                          <Form.Select
                            ...
                            label={t('所属账号')}
                            placeholder={t('请选择所属账号')}
                            optionList={unitAccountOptions}
                            loading={unitAccountLoading}
                            disabled={!Number(inputs.unit_id || 0)}
                            style={{ width: '100%' }}
                            filter={selectFilter}
                            showClear
                            onChange={handleUnitAccountChange}
                          />
                        </Col>
                      </Row>
                          ...
                          <Form.TextArea
                            ...
                            extraText={
                              <div className='flex items-center gap-2 flex-wrap'>
                                ...
                                {renderSelectUnitPlatformTokenButton()}
                                {isEdit && (
                                  <Button
                                    size='small'
                                    type='primary'
                      ...
                      {normalizeChannelGroups(inputs.groups).length > 0 && (
                        <div className='mt-3'>
                          <Text className='text-sm font-medium text-gray-500 mb-2 block'>
                            {t('分组调度配置')}
                          </Text>
                          <div className='flex flex-col gap-2'>
                            {normalizeChannelGroupConfigs(
                              inputs.groups,
                              inputs.group_configs,
                              inputs.priority,
                              inputs.weight,
                              ...
                              <div
                                key={config.group}
                                className='grid grid-cols-[minmax(120px,1fr)_120px_120px] gap-2 items-center'
                              >
      ...
      <Modal
        title={t('选择平台令牌')}
        visible={unitPlatformTokenModalVisible}
        onCancel={() => setUnitPlatformTokenModalVisible(false)}
        onOk={applySelectedUnitPlatformToken}
        okButtonProps={{ disabled: !selectedUnitPlatformTokenKey }}
        okText={t('填入渠道密钥')}
        cancelText={t('取消')}
        width={760}
        style={{ maxWidth: '92vw' }}
      >
        <div className='space-y-2'>
          <div className='flex items-start justify-between gap-3'>
            <Text type='tertiary' size='small'>
              {t(
                '这里选择的是所属单位平台里的 API 令牌，不是 NACP 当前站点的用户令牌，也不是单位账号自己的访问令牌。',
              )}
            </Text>
            <Button
              type='primary'
              ...
              size='small'
              icon={<IconPlus />}
              onClick={openCreateUnitPlatformTokenModal}
            >
              {t('新增平台令牌')}
            </Button>
          </div>
          <div className='space-y-2 max-h-[420px] overflow-auto pr-1'>
            {unitPlatformTokens.length === 0 && (
              <div className='rounded-lg border border-dashed p-4 text-center text-sm text-gray-500'>
                {t('暂无平台令牌，可以点击新增平台令牌创建。')}
              </div>
            )}
            {unitPlatformTokens.map((token) => {
              const key = String(token.key || '').trim();
              const selected = selectedUnitPlatformTokenKey === key;
              return (
                <button
                  key={token.id}
                  type='button'
                  ...
                  disabled={!key}
                  onClick={() => setSelectedUnitPlatformTokenKey(key)}
                >
                    ...
                    <Space>
                      ...
                      {token.status !== undefined && (
                        <Tag
                          ...
                          size='small'
                        >
                          {token.status === 1 ? t('启用') : t('停用')}
                        </Tag>
                      )}
                    </Space>
                  </div>
                  <div className='mt-1 text-xs text-gray-500 break-all'>
                    {getUnitPlatformTokenLabel(token)}
                  </div>
                  {token.model_limits_enabled && token.model_limits && (
                    <div className='mt-1 text-xs text-gray-500 break-all'>
                      {t('模型限制')}: {token.model_limits}
                    </div>
                  )}
                  {!key && (
                    <div className='mt-1 text-xs text-red-400'>
                      {t('该平台未返回完整令牌密钥，不能填入渠道')}
                    </div>
                  )}
                </button>
              );
      ...
      <Modal
        title={t('新增平台令牌')}
        visible={unitPlatformTokenCreateVisible}
        onCancel={() => setUnitPlatformTokenCreateVisible(false)}
        onOk={() => unitPlatformTokenCreateFormApiRef.current?.submitForm()}
        confirmLoading={unitPlatformTokenCreating}
        okText={t('创建并选中')}
        cancelText={t('取消')}
        width={720}
        style={{ maxWidth: '92vw' }}
      >
        <Form
          initValues={getUnitPlatformTokenCreateInitValues()}
          getFormApi={(api) =>
            (unitPlatformTokenCreateFormApiRef.current = api)
          }
          onSubmit={submitCreateUnitPlatformToken}
        >
          {({ values }) => (
            <div className='space-y-3'>
              ...
              <Form.Select
                ...
                optionList={(unitPlatformTokenOptions.groups || []).map(
                  (group) => ({
              ...
              <Form.Select
                ...
                multiple
                optionList={(unitPlatformTokenOptions.models || []).map(
                  (model) => ({
                    label: model,
                    value: model,
                  }),

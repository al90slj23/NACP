...
```text
sft_total_timeout = 30s
```

超时后：

1. 不再继续启动新的同渠道重试、候选探测等待和备用渠道正式请求。
2. 返回最后一个真实正式请求错误。
3. 生成 `52` 终态事件。
4. 生成或修复 `50` 链路摘要。
5. 后到的探测或请求结果只能作为迟到结果记录，不得改变已经返回给用户的终态。

当前实现边界：总时限会限制“继续调度下一步”和“等待预热探测”的时间，但不会强行中断一个已经发出的正式上游请求；强中断需要进一步把 SFT deadline 注入请求 context，需单独验证流式请求和长响应场景。

...
func (Adapter) Type() string { return "gemini" }

func (Adapter) Aliases() []string { return []string{"google"} }
...
func (Adapter) Detect(_ context.Context, _ *http.Client, baseURL string) (unitplatform.Result, bool) {
	...
	if strings.Contains(normalized, "generativelanguage.googleapis.com") ||
		strings.Contains(normalized, "googleapis.com/v1beta/openai") ||
		strings.Contains(normalized, "gemini.google.com") {
		return unitplatform.Detected("gemini", baseURL, "根据 URL 命中 Gemini API。"), true
	}
...
func (Adapter) FetchTokenOptions(ctx context.Context, credentials unitplatform.Credentials) (*unitplatform.UnitPlatformTokenOptions, error) {
	models, err := upcommon.FetchGeminiModels(ctx, credentials.BaseURL, credentials.APIKey)
	if err != nil {
		return nil, err
	}
